...
def test_dkim_authentication(playwright: Playwright) -> None:
    ...
    page.get_by_role("button", name="Next").click()
    time.sleep(5)
    print("Signed ito the landing page....")
    ...
    for domain in domains:
        ...
        print("Generating dns recod to the domain: ", domain)
        page.get_by_role("button", name="Generate new record").click()

        # Extract data values into a list
        # options = page.locator(".ur98K .jgvuAb span").evaluate(['span => span.getAttribute("data-value")'])
        # options = page.locator(".ur98K .jgvuAb [data-value]").evaluate(['elements => elements.map(span => span.textContent)'])
        # options = page.locator(".ur98K .jgvuAb [data-value]").nth(2).evaluate(['span => span.textContent'])
        # options = page.locator(".ur98K .jgvuAb [data-value='1024']").nth(2).inner_text()
...
with sync_playwright() as playwright:
    test_dkim_authentication_result = test_dkim_authentication(playwright)
